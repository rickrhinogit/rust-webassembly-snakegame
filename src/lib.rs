...
impl World {
    ...
    fn gen_next_snake_cell(&self, direction: &Direction) -> SnakeCell {
        ...
        return match direction {
            Direction::Right => {
                let threshhold = (row + 1) * self.width;
                if snake_idx + 1 == threshhold {
                    SnakeCell(threshhold - self.width)
                } else {
                    SnakeCell(snake_idx + 1)
                }
            }
            Direction::Left => {
                let threshhold = row * self.width;
                if snake_idx == threshhold {
                    SnakeCell(threshhold + (self.width - 1))
                } else {
                    SnakeCell(snake_idx - 1)
                }
            }
            Direction::Up => {
                let threshhold = snake_idx - (row * self.width);
                if snake_idx == threshhold {
                    SnakeCell((self.size - self.width) + threshhold)
                } else {
                    SnakeCell(snake_idx - self.width)
                }
            }
            Direction::Down => {
                let threshhold = snake_idx + ((self.width - row) * self.width);
                if snake_idx + self.width == threshhold {
                    SnakeCell(threshhold - ((row + 1) * self.width))
                } else {
                    SnakeCell(snake_idx + self.width)
                }
            }
